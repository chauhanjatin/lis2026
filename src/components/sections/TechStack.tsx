  ...
  return (
    ...
    >
      {Icon ? (
        ...
        <span className={`integration-mark ${item.color}`}>{item.mark}</span>
      )}
    </motion.div>
  );
}
      ...
      <Container>
        ...
        >
          <p className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.12em] text-[#202224]">
            <span className="h-2 w-2 rounded-full bg-[#16181a]" /> Technologies
          </p>
          ...
          <div className="integration-rail integration-rail-left">
            ...
            >
              {leftRail.map((item, index) => (
                <IntegrationCard key={`${item.name}-${index}`} item={item} />
              ))}
            </motion.div>
          </div>
          <motion.div
